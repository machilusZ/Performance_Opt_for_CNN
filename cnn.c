...
static vol_t* copy_vol(vol_t* dest, vol_t* src) {
  double* dest_weights = dest->w;
  double* src_weights = src->w;
  int dest_sx = dest->sx, dest_sy = dest->sy, dest_depth = dest->depth;
  int src_sx = src->sx, src_sy = src->sy, src_depth = src->depth;
  int dest_index, src_index, prefix1, prefix2;
  for (int y = 0; y < dest_sy; y++) {
    prefix1 = dest_sx * y * dest_depth;
    prefix2 = src_sx * y * dest_depth;
    for (int x = 0; x < dest_sx; x++) {
      dest_index = prefix1 + x * dest_depth;
      src_index = prefix2 + x * dest_depth;
      for (int z = 0; z < dest_depth; z++) {
        dest_weights[dest_index + z] = src_weights[src_index + z];
      }
    }
  }
...
conv_layer_t* make_conv_layer(int in_sx, int in_sy, int in_depth,
  ...
  // computed
  double out_sx_sy = floor((in_sx + pad * 2 - sx) / stride + 1);
  l->out_sx = out_sx_sy;
  l->out_sy = out_sx_sy;

  l->filters = (vol_t**)malloc(sizeof(vol_t*)*filters);
  for (int i = 0; i < filters; i++) {
    l->filters[i] = make_vol(sx, sx, in_depth, 0.0);
  }

  l->bias = 0.0;
  l->biases = make_vol(1, 1, filters, 0.0);

  return l;
}

// -------------------- MY HELPER FUNCTOINS END ----------------------

void conv_forward_3(conv_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
    int xy_stride = l->stride, lPad = -l->pad, lout_sx = l->out_sx;
    int lout_sy = l->out_sy, loutdepth = l->out_depth;
    double* lbiasesw = l->biases->w;
    vol_t** lfilters = l->filters;

    double *Aw, *Vw, *fw, a;
    int Asx, Adepth, V_sx, V_sy, Vdepth, x, y, A_row_index_start,A_depth_index_start;
    int fsx, fsy, fdepth, ox, oy, f_row_index_start, V_row_index_start;
    int f_depth_index_start, V_depth_index_start;
    int i, ay, ax, d, fx, fy;
    ...
    for (i = start; i <= end; i++) {
        ...
        Vw = V->w;
        y = lPad;
        for(ay = 0; ay < lout_sy; y += xy_stride, ay++) {
            x = lPad;
            A_row_index_start = Asx * ay;
            for(ax = 0; ax < lout_sx; x += xy_stride, ax++) {
              
                A_depth_index_start = (A_row_index_start + ax) * Adepth;

                for(d = 0; d < loutdepth; d++) {
                    vol_t* f = lfilters[d];
                    fsx = f->sx;
                    fsy = f->sy;
                    fdepth = f->depth;
                    fw = f->w;
                    a = 0.0;
                    for(fy = 0; fy < fsy && fy < V_sy - y; fy++) {
                        oy = y + fy;
                        f_row_index_start = fsx * fy;
                        V_row_index_start = V_sx * oy;
                        if (oy >= 0)
                        {
                            for(fx = 0; fx < fsx && fx < V_sx - x; fx++) {
                              ox = x + fx;
                              if(ox >=0) {
                                  f_depth_index_start = (f_row_index_start + fx) * fdepth;
                                  V_depth_index_start = (V_row_index_start + ox) * Vdepth;
                                  a += fw[f_depth_index_start] * Vw[V_depth_index_start];
                                  a += fw[f_depth_index_start+1] * Vw[V_depth_index_start+1];
                                  a += fw[f_depth_index_start+2] * Vw[V_depth_index_start+2];
                              }
                          }
                        }
                    }
                    a += lbiasesw[d];
                    Aw[A_depth_index_start + d] = a;
                        ...
                        if (oy >= 0)
                        {
                            for(fx = 0; fx < fsx && fx < V_sx - x; fx++) {
                              ...
                              if(ox >=0) {
                                  ...
                                  f_depth_index_start = (f_row_index_start + fx) * fdepth;
                                  V_depth_index_start = (V_row_index_start + ox) * Vdepth;
                                  sums = _mm256_add_pd(sums, _mm256_mul_pd(_mm256_loadu_pd(fw + f_depth_index_start), \
                                                                            _mm256_loadu_pd(Vw + V_depth_index_start)));
                                  sums = _mm256_add_pd(sums, _mm256_mul_pd(_mm256_loadu_pd(fw + f_depth_index_start + 4), \
                                                                    _mm256_loadu_pd(Vw + V_depth_index_start + 4)));
                                  sums = _mm256_add_pd(sums, _mm256_mul_pd(_mm256_loadu_pd(fw + f_depth_index_start + 8), \
                                                                            _mm256_loadu_pd(Vw + V_depth_index_start + 8)));
                                  sums = _mm256_add_pd(sums, _mm256_mul_pd(_mm256_loadu_pd(fw + f_depth_index_start + 12), \
                                                                            _mm256_loadu_pd(Vw + V_depth_index_start + 12)));
                              }
                          }
                        }
                    }
                    _mm256_store_pd(result, sums);
                    a += result[0] + result[1] + result[2] + result[3] + lbiasesw[d];
                    Aw[A_depth_index_start + d] = a;
                }
            }
        }
                        ...
                        if (oy >= 0)
                        {
                            for(fx = 0; fx < fsx && fx < V_sx - x; fx++) {
                              ...
                              if(ox >=0) {
                                  ...
                                  sums = _mm256_add_pd(sums, _mm256_mul_pd(_mm256_loadu_pd(fw + f_depth_index_start + 12), \
                                                                            _mm256_loadu_pd(Vw + V_depth_index_start + 12)));
                                  sums = _mm256_add_pd(sums, _mm256_mul_pd(_mm256_loadu_pd(fw + f_depth_index_start + 16), \
                                                                            _mm256_loadu_pd(Vw + V_depth_index_start + 16)));
                              }
                          }
                        }
                    }
    ...
    for (int x = 0; x < sx; x++)
      for (int y = 0; y < sy; y++)
        for (int z = 0; z < depth; z++) {
          fscanf(fin, "%lf", &val);
          set_vol(lfilters[d], x, y, z, val);
        }

  for(int d = 0; d < l->out_depth; d++) {
    fscanf(fin, "%lf", &val);
    set_vol(lbiases, 0, 0, d, val);
  }
...
batch_t* make_batch(network_t* old_net, int size) {
  ...
  for (int i = 0; i < LAYERS+1; i++) {
    ...
    sx = old_net_v[i]->sx;
    sy = old_net_v[i]->sy;
    depth = old_net_v[i]->depth;
    for (int j = 0; j < size; j++) {
      out[i][j] = make_vol(sx, sy, depth, 0.0);
    }
  }
  return out;
}
...
void net_classify_cats(network_t* net, vol_t** input, double* output, int n) {
  int batchsize = 12;
  int edge = n / batchsize * batchsize;
  #pragma omp parallel for
  for (int i = 0; i < edge; i+=batchsize) {
    batch_t* batch = make_batch(net, batchsize);
    for (int j = 0; j < batchsize; j+=4)
    {
      copy_vol(batch[0][j], input[i+j]);
      copy_vol(batch[0][j+1], input[i+j+1]);
      copy_vol(batch[0][j+2], input[i+j+2]);
      copy_vol(batch[0][j+3], input[i+j+3]);
    }
    net_forward(net, batch, 0, batchsize - 1);
    for (int j = 0; j < batchsize; j+=4)
    {
      output[i+j] = batch[11][j]->w[CAT_LABEL]; 
      output[i+j+1] = batch[11][j+1]->w[CAT_LABEL]; 
      output[i+j+2] = batch[11][j+2]->w[CAT_LABEL]; 
      output[i+j+3] = batch[11][j+3]->w[CAT_LABEL]; 
    }
    free_batch(batch, batchsize);
  }

  #pragma omp parallel for
  for (int i = edge; i < n / 4 * 4; i+=4) {
